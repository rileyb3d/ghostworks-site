      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="border-b border-white/[0.08] text-left font-display text-[11px] font-medium uppercase tracking-[0.18em] text-zinc-500">
            ...
            <th className="py-3 text-right font-medium">Link</th>
          </tr>
        </thead>
        <tbody>
          {invoices.map((inv) => {
            const status = inv.status ?? "draft";
            const tone = STATUS_TONE[status] ?? "bg-zinc-700/30 text-zinc-300";
            const label = STATUS_LABEL[status] ?? status;
            const description =
              inv.lines.data[0]?.description ?? inv.description ?? "—";
            const customerEmail =
              inv.customer_email ??
              (typeof inv.customer === "object" && inv.customer && !inv.customer.deleted
                ? inv.customer.email ?? "—"
                : "—");
